...
class MainWindowPocketGis(QMainWindow):
    def __init__(self, app, w=512, h=512, minW=256, minH=256, winTitle='My Window', iconPath='', parent=None):
        super(MainWindowPocketGis, self).__init__(parent)  # super().__init__()
        self.app = app
        ...
        self._centralWidget = WidgetCentral(w=512, h=512, minW=64, minH=64, maxW=2048, maxH=2048)
        ...
        # ***** ACTIONS ***** #
        self.actionExit = QAction(QIcon(projFlags.ICON_CLOSE_PATH), 'Exit' + self.setSpaces(_INT_SPACES))  # Exit
...
def exec_app(w=512, h=512, minW=256, minH=256, winTitle='My Window', iconPath=''):
    ...
    mainWin = MainWindowPocketGis(myApp, w=w, h=h, minW=minW, minH=minH, winTitle=winTitle,
